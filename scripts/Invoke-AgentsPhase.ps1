...
.PARAMETER ProjectRoot
    Path to the HyperCode-V2.0 repo root. Defaults to current directory.
...
function Test-Prerequisites {
    ...
    if (Test-Path (Join-Path $ProjectRoot 'backend')) {
        ...
        Write-Fail "backend/ directory not found at $ProjectRoot" "Run from the HyperCode-V2.0 repo root or set -ProjectRoot"
    }
